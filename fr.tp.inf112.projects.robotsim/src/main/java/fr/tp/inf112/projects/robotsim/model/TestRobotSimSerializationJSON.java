...
public class TestRobotSimSerializationJSON {
    ...
    public void testSerialization()
        ...
        assertEquals(factory.getName(), roundTrip.getName());
        assertEquals(factory.getComponents().size(), roundTrip.getComponents().size());

        // Check first room
        Component restoredRoom = roundTrip.getComponents().get(0);
        assertTrue(restoredRoom instanceof Room);
        assertEquals("Production Room 1", restoredRoom.getName());
        assertEquals(75, restoredRoom.getWidth());
        assertEquals(75, restoredRoom.getHeight());

        // Check nested area
        Room room = (Room) restoredRoom;
        assertEquals(1, room.getAreas().size());
        Area restoredArea = room.getAreas().get(0);
        assertEquals("Production Area 1", restoredArea.getName());
        assertEquals(50, restoredArea.getWidth());
        assertEquals(50, restoredArea.getHeight());

        // Check machine inside area
        Machine restoredMachine = restoredArea.getMachine();
        assertEquals("Machine 1", restoredMachine.getName());
        assertEquals(15, restoredMachine.getWidth());
        assertEquals(15, restoredMachine.getHeight());
